...
( function( $ ) {
	...
	if (!reisen_previewer_vars['need_refresh']) {
		var $style = $('#reisen-customizer-css');
		// Prepare inline styles in the preview window
		if ( $style.length == 0 ) {
			$style = $('head').append( '<'+'style type="text/css" id="reisen-customizer-css"></'+'style'+'>' ).find('#reisen-customizer-css');
		}
	}
	...
	api.bind( 'preview-ready', function() {
		...
		api.preview.bind( 'refresh-preview', function( ) {
			"use strict";
			reisen_set_cookie(reisen_previewer_vars['options_prefix']+'_compile_less', 1);
		} );
