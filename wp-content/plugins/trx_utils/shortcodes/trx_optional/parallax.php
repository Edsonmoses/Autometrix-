...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('reisen_sc_parallax_theme_setup')) {
	add_action( 'reisen_action_before_init_theme', 'reisen_sc_parallax_theme_setup' );
	function reisen_sc_parallax_theme_setup() {
		add_action('reisen_action_shortcodes_list', 		'reisen_sc_parallax_reg_shortcodes');
		if (function_exists('reisen_exists_visual_composer') && reisen_exists_visual_composer())
			add_action('reisen_action_shortcodes_list_vc','reisen_sc_parallax_reg_shortcodes_vc');
	}
}
...
if (!function_exists('reisen_sc_parallax')) {	
	function reisen_sc_parallax($atts, $content=null){	
		...
		if ($bg_video!='') {
			...
			$bg_video_width = !empty($width) && reisen_substr($width, -1) >= '0' && reisen_substr($width, -1) <= '9'  ? $width : 1280;
		...
		$css .= reisen_get_css_dimensions_from_values($width, $height)
			...
			;
		$output = (reisen_param_is_on($gap) ? reisen_gap_start() : '')
			...
			. '</div>'
			. '</div>'
			. (reisen_param_is_on($gap) ? reisen_gap_end() : '');
		return apply_filters('reisen_shortcode_output', $output, 'trx_parallax', $atts, $content);
	}
	add_shortcode('trx_parallax', 'reisen_sc_parallax');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'reisen_sc_parallax_reg_shortcodes' ) ) {
	//add_action('reisen_action_shortcodes_list', 'reisen_sc_parallax_reg_shortcodes');
	function reisen_sc_parallax_reg_shortcodes() {
	
		reisen_sc_map("trx_parallax", array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'reisen_sc_parallax_reg_shortcodes_vc' ) ) {
	//add_action('reisen_action_shortcodes_list_vc', 'reisen_sc_parallax_reg_shortcodes_vc');
	function reisen_sc_parallax_reg_shortcodes_vc() {
		...
		class WPBakeryShortCode_Trx_Parallax extends Reisen_Vc_ShortCodeCollection {}
	}
}
?>