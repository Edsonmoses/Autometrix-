...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('reisen_sc_form_theme_setup')) {
	add_action( 'reisen_action_before_init_theme', 'reisen_sc_form_theme_setup' );
	function reisen_sc_form_theme_setup() {
		add_action('reisen_action_shortcodes_list', 		'reisen_sc_form_reg_shortcodes');
		if (function_exists('reisen_exists_visual_composer') && reisen_exists_visual_composer())
			add_action('reisen_action_shortcodes_list_vc','reisen_sc_form_reg_shortcodes_vc');
	}
}
...
if (!function_exists('reisen_sc_form')) {	
	function reisen_sc_form($atts, $content = null) {
		...
		reisen_enqueue_messages();	// Load core messages
	
		reisen_storage_set('sc_form_data', array(
			'id' => $id,
            'counter' => 0
            )
        );
		...
		$output = '<div ' . ($id ? ' id="'.esc_attr($id).'_wrap"' : '')
			...
			.'<div ' . ($id ? ' id="'.esc_attr($id).'"' : '') 
				. ' class="sc_form'
					...
					. (!empty($align) && !reisen_param_is_off($align) ? ' align'.esc_attr($align) : '') 
				...
				. '>'
					. (!empty($subtitle) 
						? '<h6 class="sc_form_subtitle sc_item_subtitle">' . trim(reisen_strmacros($subtitle)) . '</h6>' 
						: '')
					. (!empty($title) 
						? '<h3 class="sc_form_title sc_item_title' . (empty($description) ? ' sc_item_title_without_descr' : ' sc_item_title_without_descr') . '">' . trim(reisen_strmacros($title)) . '</h3>'
						: '')
					. (!empty($description) 
						? '<div class="sc_form_descr sc_item_descr">' . trim(reisen_strmacros($description)) . ($style == 1 ? do_shortcode('[trx_socials size="tiny" shape="round"][/trx_socials]') : '') . '</div>' 
						: '');
		
		$output .= reisen_show_post_layout(array(
												'layout' => $style,
												'id' => $id,
												'action' => $action,
												'content' => $content,
		...
		return apply_filters('reisen_shortcode_output', $output, 'trx_form', $atts, $content);
	}
	add_shortcode("trx_form", "reisen_sc_form");
}

if (!function_exists('reisen_sc_form_item')) {	
	function reisen_sc_form_item($atts, $content=null) {
		if (reisen_in_shortcode_blogger()) return '';
		extract(reisen_html_decode(shortcode_atts( array(
			// Individual params
			"type" => "text",
		...
		reisen_storage_inc_array('sc_form_data', 'counter');
	
		$class .= ($class ? ' ' : '') . reisen_get_css_position_as_classes($top, $right, $bottom, $left);
		if (empty($id)) $id = reisen_storage_get_array('sc_form_data', 'id').'_'.reisen_storage_get_array('sc_form_data', 'counter');
		...
		if ($type == 'textarea')
			...
			wp_enqueue_script( 'jquery-picker', reisen_get_file_url('/js/picker/picker.js'), array('jquery'), null, true );
			wp_enqueue_script( 'jquery-picker-date', reisen_get_file_url('/js/picker/picker.date.js'), array('jquery'), null, true );
			...
			wp_enqueue_script( 'jquery-picker', reisen_get_file_url('/js/picker/picker.js'), array('jquery'), null, true );
			wp_enqueue_script( 'jquery-picker-time', reisen_get_file_url('/js/picker/picker.time.js'), array('jquery'), null, true );
		...
		return apply_filters('reisen_shortcode_output', $output, 'trx_form_item', $atts, $content);
	}
	add_shortcode('trx_form_item', 'reisen_sc_form_item');
}

// AJAX Callback: Send contact form data
if ( !function_exists( 'reisen_sc_form_send' ) ) {
	function reisen_sc_form_send() {
	
		if ( !wp_verify_nonce( trx_utils_get_value_gp('nonce'), admin_url('admin-ajax.php') ) )
			die();
	
		$response = array('error'=>'');
		if (!($contact_email = reisen_get_theme_option('contact_email')) && !($contact_email = reisen_get_theme_option('admin_email'))) 
			$response['error'] = esc_html__('Unknown admin email!', 'trx_utils');
		else {
			$type = reisen_substr($_REQUEST['type'], 0, 7);
			parse_str($_POST['data'], $post_data);

			if (in_array($type, array('form_1', 'form_2'))) {
				$user_name	= reisen_strshort($post_data['username'],	100);
				$user_email	= reisen_strshort($post_data['email'],	100);
				$user_subj	= reisen_strshort($post_data['subject'],	100);
				$user_msg	= reisen_strshort($post_data['message'],	reisen_get_theme_option('message_maxlength_contacts'));
			...
			$mail = reisen_get_theme_option('mail_function');
			if (!@$mail($contact_email, $subj, apply_filters('reisen_filter_form_send_message', $msg))) {
...
if ( !function_exists( 'reisen_sc_form_show_fields' ) ) {
	function reisen_sc_form_show_fields($fields) {
...
-------------------------------------------------------------------- */
if ( !function_exists( 'reisen_sc_form_reg_shortcodes' ) ) {
	//add_action('reisen_action_shortcodes_list', 'reisen_sc_form_reg_shortcodes');
	function reisen_sc_form_reg_shortcodes() {
	
		$pages = reisen_get_list_pages(false);

		reisen_sc_map("trx_form", array(
			...
			"params" => array(
				...
				"style" => array(
					...
					"divider" => true,
					"value" => 'form_custom',
					"options" => reisen_get_sc_param('forms'),
					"type" => "checklist"
				), 
				"scheme" => array(
			...
			"children" => array(
				...
				"params" => array(
					"type" => array(
						...
						"type" => "checklist",
						"dir" => "horizontal",
						"options" => reisen_get_sc_param('field_types')
					), 
					"name" => array(
					...
					"label_position" => array(
						...
						"type" => "checklist",
						"dir" => "horizontal",
						"options" => reisen_get_sc_param('label_positions')
					), 
					"top" => reisen_get_sc_param('top'),
					"bottom" => reisen_get_sc_param('bottom'),
					"left" => reisen_get_sc_param('left'),
...
-------------------------------------------------------------------- */
if ( !function_exists( 'reisen_sc_form_reg_shortcodes_vc' ) ) {
	//add_action('reisen_action_shortcodes_list_vc', 'reisen_sc_form_reg_shortcodes_vc');
	function reisen_sc_form_reg_shortcodes_vc() {
		...
		vc_map( array(
			...
			"params" => array(
				array(
					...
					"class" => "",
					"std" => "form_custom",
					"value" => array_flip(reisen_get_sc_param('forms')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "scheme",
		...
		vc_map( array(
			...
			"params" => array(
				array(
					...
					"admin_label" => true,
					"class" => "",
					"value" => array_flip(reisen_get_sc_param('field_types')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "name",
				...
				array(
					...
					"class" => "",
					"value" => array_flip(reisen_get_sc_param('label_positions')),
					"type" => "dropdown"
				),
				reisen_get_vc_param('id'),
				reisen_get_vc_param('class'),
		...
		class WPBakeryShortCode_Trx_Form extends Reisen_Vc_ShortCodeCollection {}
		class WPBakeryShortCode_Trx_Form_Item extends Reisen_Vc_ShortCodeItem {}
	}
}
?>