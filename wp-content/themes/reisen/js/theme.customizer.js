...
function reisen_theme_customizer(option, val) {
    "use strict";
	var custom_style = '';
	...
	if (option == 'body_style') {
		...
		var params = REISEN_STORAGE['co_add_params']!=undefined ? REISEN_STORAGE['co_add_params'] : {};
		params[option] = val;
	...
	if (custom_style != '') {
		var styles = jQuery('#reisen-customizer-styles-'+option).length > 0 ? jQuery('#reisen-customizer-styles-'+option) : '';
		if (styles.length == 0)
			jQuery('head').append('<style id="reisen-customizer-styles-'+option+'" type="text/css">'+custom_style+'</style>');
		else
