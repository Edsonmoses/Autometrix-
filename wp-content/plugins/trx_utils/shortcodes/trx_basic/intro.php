...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('reisen_sc_intro_theme_setup')) {
	add_action( 'reisen_action_before_init_theme', 'reisen_sc_intro_theme_setup' );
	function reisen_sc_intro_theme_setup() {
		add_action('reisen_action_shortcodes_list', 		'reisen_sc_intro_reg_shortcodes');
		if (function_exists('reisen_exists_visual_composer') && reisen_exists_visual_composer())
			add_action('reisen_action_shortcodes_list_vc','reisen_sc_intro_reg_shortcodes_vc');
	}
}
...
if (!function_exists('reisen_sc_intro')) {	
	function reisen_sc_intro($atts, $content=null){	
		...
		$output = '<div '.(!empty($url) ? 'data-href="'.esc_url($url).'"' : '') 
					...
					. '<div class="sc_intro_inner '.($style ? ' sc_intro_style_' . esc_attr($style) : '').'"'.(!empty($content_width) ? ' style="width:'.esc_attr($content_width).';"' : '').'>'
						...
						. '<div class="sc_intro_content">'
							. (!empty($subtitle) && $style!=4 && $style!=5 ? '<h6 class="sc_intro_subtitle">' . trim(reisen_strmacros($subtitle)) . '</h6>' : '')
							. (!empty($title) ? '<h2 class="sc_intro_title">' . trim(reisen_strmacros($title)) . '</h2>' : '')
							. (!empty($description) && $style!=1 ? '<div class="sc_intro_descr">' . trim(reisen_strmacros($description)) . '</div>' : '')
		...
		return apply_filters('reisen_shortcode_output', $output, 'trx_intro', $atts, $content);
	}
	add_shortcode('trx_intro', 'reisen_sc_intro');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'reisen_sc_intro_reg_shortcodes' ) ) {
	//add_action('reisen_action_shortcodes_list', 'reisen_sc_intro_reg_shortcodes');
	function reisen_sc_intro_reg_shortcodes() {
	
		reisen_sc_map("trx_intro", array(
			...
			"params" => array(
				"style" => array(
					...
					"value" => "1",
					"type" => "checklist",
					"options" => reisen_get_list_styles(1, 5)
				),
				"align" => array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'reisen_sc_intro_reg_shortcodes_vc' ) ) {
	//add_action('reisen_action_shortcodes_list_vc', 'reisen_sc_intro_reg_shortcodes_vc');
	function reisen_sc_intro_reg_shortcodes_vc() {
		...
		vc_map( array(
			...
			"params" => array(
				array(
					...
					"class" => "",
					"admin_label" => true,
					"value" => array_flip(reisen_get_list_styles(1, 5)),
					"type" => "dropdown"
				),
				array(
					"param_name" => "align",
		...
		class WPBakeryShortCode_Trx_Intro extends Reisen_Vc_ShortCodeSingle {}
	}
}
?>