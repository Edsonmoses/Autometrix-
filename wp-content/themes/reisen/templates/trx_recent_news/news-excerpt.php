...
if ( !function_exists( 'reisen_template_news_excerpt_theme_setup' ) ) {
	add_action( 'reisen_action_before_init_theme', 'reisen_template_news_excerpt_theme_setup', 1 );
	function reisen_template_news_excerpt_theme_setup() {
		reisen_add_template(array(
			'layout' => 'news-excerpt',
			'template' => 'news-excerpt',
			'mode'   => 'news',
			'title'  => esc_html__('Recent News /Style Excerpt/', 'reisen'),
...
// Template output
if ( !function_exists( 'reisen_template_news_excerpt_output' ) ) {
	function reisen_template_news_excerpt_output($post_options, $post_data) {
				...
				if ( !in_array($post_data['post_format'], array('link', 'aside', 'status', 'quote')) ) {
					...
					if ($show_title) {
						if (!isset($post_options['links']) || $post_options['links']) {
							?>
							<h5 class="post_title entry-title"><a href="<?php echo esc_url($post_data['post_link']); ?>"><?php reisen_show_layout(wp_kses_data($post_data['post_title'])); ?></a></h5>
							<?php
						} else {
							?>
							<h5 class="post_title entry-title"><?php reisen_show_layout(wp_kses_data($post_data['post_title'])); ?></h5>
							<?php
						}
					}
						...
						if ($post_data['post_excerpt']) {
							echo in_array($post_data['post_format'], array('quote', 'link', 'chat', 'aside', 'status')) 
									...
									: wpautop(reisen_strshort($post_data['post_excerpt'], isset($post_options['descr']) 
																								? $post_options['descr'] 
																								: reisen_get_custom_option('post_excerpt_maxlength')
																)
											);
						}
					}
