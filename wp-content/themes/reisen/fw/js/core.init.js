...
function reisen_ready_actions() {
	...
	jQuery( document.body ).on( 'click', '.remove_from_cart_button', function (){
			var product_id = jQuery(this).attr('data-product_id');
			jQuery(".add_to_cart_button[data-product_id='" + product_id +"']").removeClass('added');
			jQuery(".add_to_cart_button[data-product_id='" + product_id +"']").next().remove();
	});
...
function reisen_init_isotope() {
	...
	jQuery('.isotope_wrap:not(.inited)').each(function() {
		...
		// Init shortcodes
		if(window.reisen_sc_init) reisen_sc_init(isotope_container);
...
function reisen_init_appended_isotope(posts_container, filters) {
	...
	if(window.reisen_sc_init) reisen_sc_init(posts_container);
	// Get added elements
