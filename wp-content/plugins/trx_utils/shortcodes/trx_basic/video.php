...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('reisen_sc_video_theme_setup')) {
	add_action( 'reisen_action_before_init_theme', 'reisen_sc_video_theme_setup' );
	function reisen_sc_video_theme_setup() {
		add_action('reisen_action_shortcodes_list', 		'reisen_sc_video_reg_shortcodes');
		if (function_exists('reisen_exists_visual_composer') && reisen_exists_visual_composer())
			add_action('reisen_action_shortcodes_list_vc','reisen_sc_video_reg_shortcodes_vc');
	}
}
...
if (!function_exists('reisen_sc_video')) {	
	function reisen_sc_video($atts, $content = null) {
		...
		if (empty($height) && empty($width)) {
			$width='100%';
			if (reisen_param_is_off(reisen_get_custom_option('substitute_video'))) $height="400";
		}
		$ed = reisen_substr($width, -1);
		...
		$url = $src!='' ? $src : $url;
		if ($image!='' && reisen_param_is_off($image))
			$image = '';
		else {
			if (reisen_param_is_on($autoplay) && is_singular() && !reisen_storage_get('blog_streampage'))
				$image = '';
			else {
				if ($image > 0) {
				...
				if ($bg_image) {
					...
					if (!is_single() || !empty($image)) $image = reisen_get_resized_image_url(empty($image) ? get_the_ID() : $image, $thumb_sizes['w'], $thumb_sizes['h'], null, false, false, false);
				} else
					if (!is_single() || !empty($image)) $image = reisen_get_resized_image_url(empty($image) ? get_the_ID() : $image, $ed!='%' ? $width : null, $height);
				if (empty($image) && (!is_singular() || reisen_storage_get('blog_streampage')))	// || reisen_param_is_off($autoplay)))
					$image = reisen_get_video_cover_image($url);
			}
		}
		if ($bg_image > 0) {
		...
		$url = reisen_get_video_player_url($src!='' ? $src : $url);
		...
		$video = '<video' . ($id ? ' id="' . esc_attr($id) . '"' : '') 
			...
			. (($image && reisen_param_is_on(reisen_get_custom_option('substitute_video'))) || (reisen_param_is_on($autoplay) && is_singular() && !reisen_storage_get('blog_streampage')) ? ' autoplay="autoplay"' : '') 
			. ' controls="controls" loop="loop"'
			. '>'
			. '</video>';
		if (reisen_param_is_off(reisen_get_custom_option('substitute_video'))) {
			if (reisen_param_is_on($frame)) $video = reisen_get_video_frame($video, $image, $css, $css_bg);
		} else {
		...
		return apply_filters('reisen_shortcode_output', $video, 'trx_video', $atts, $content);
	}
	add_shortcode("trx_video", "reisen_sc_video");
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'reisen_sc_video_reg_shortcodes' ) ) {
	//add_action('reisen_action_shortcodes_list', 'reisen_sc_video_reg_shortcodes');
	function reisen_sc_video_reg_shortcodes() {
	
		reisen_sc_map("trx_video", array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'reisen_sc_video_reg_shortcodes_vc' ) ) {
	//add_action('reisen_action_shortcodes_list_vc', 'reisen_sc_video_reg_shortcodes_vc');
	function reisen_sc_video_reg_shortcodes_vc() {
		...
		class WPBakeryShortCode_Trx_Video extends Reisen_Vc_ShortCodeSingle {}
	}
}
?>