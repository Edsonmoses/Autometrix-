...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('reisen_sc_title_theme_setup')) {
	add_action( 'reisen_action_before_init_theme', 'reisen_sc_title_theme_setup' );
	function reisen_sc_title_theme_setup() {
		add_action('reisen_action_shortcodes_list', 		'reisen_sc_title_reg_shortcodes');
		if (function_exists('reisen_exists_visual_composer') && reisen_exists_visual_composer())
			add_action('reisen_action_shortcodes_list_vc','reisen_sc_title_reg_shortcodes_vc');
	}
}
...
if (!function_exists('reisen_sc_title')) {	
	function reisen_sc_title($atts, $content=null){	
		...
		$css .= reisen_get_css_dimensions_from_values($width)
			.($align && $align!='none' && !reisen_param_is_inherit($align) ? 'text-align:' . esc_attr($align) .';' : '')
		...
		$pic = $style!='iconed' 
			...
			: '<span class="sc_title_icon sc_title_icon_'.esc_attr($position).'  sc_title_icon_'.esc_attr($image_size).($icon!='' && $icon!='none' ? ' '.esc_attr($icon) : '').'"'.'>'
				...
				.(empty($picture) && $image && $image!='none' ? '<img src="'.esc_url(reisen_strpos($image, 'http')===0 ? $image : reisen_get_file_url('images/icons/'.($image).'.png')).'" alt="'.esc_html($image).'" />' : '')
				.'</span>';
		$output = '<h' . esc_attr($type) . ($id ? ' id="'.esc_attr($id).'"' : '')
				. ' class="sc_title sc_title_'.esc_attr($style)
					.($align && $align!='none' && !reisen_param_is_inherit($align) ? ' sc_align_' . esc_attr($align) : '')
				...
				. '</h' . esc_attr($type) . '>';
		return apply_filters('reisen_shortcode_output', $output, 'trx_title', $atts, $content);
	}
	add_shortcode('trx_title', 'reisen_sc_title');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'reisen_sc_title_reg_shortcodes' ) ) {
	//add_action('reisen_action_shortcodes_list', 'reisen_sc_title_reg_shortcodes');
	function reisen_sc_title_reg_shortcodes() {
	
		reisen_sc_map("trx_title", array(
			...
			"params" => array(
				...
				"image" => array(
					...
					"type" => "images",
					"size" => "small",
					"options" => reisen_get_sc_param('images')
				),
				"picture" => array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'reisen_sc_title_reg_shortcodes_vc' ) ) {
	//add_action('reisen_action_shortcodes_list_vc', 'reisen_sc_title_reg_shortcodes_vc');
	function reisen_sc_title_reg_shortcodes_vc() {
		...
		vc_map( array(
			...
			"params" => array(
				...
				array(
					...
					"value" => reisen_get_sc_param('images'),
					"type" => "dropdown"
				),
				array(
					"param_name" => "picture",
		...
		class WPBakeryShortCode_Trx_Title extends Reisen_Vc_ShortCodeSingle {}
	}
}
?>