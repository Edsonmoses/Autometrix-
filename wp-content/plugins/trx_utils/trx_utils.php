...
if ( !function_exists( 'trx_utils_core_frontend_scripts' ) ) {
    function trx_utils_core_frontend_scripts() {
        // Google map
        if ( reisen_get_custom_option('show_googlemap')=='yes' && reisen_get_theme_option('api_google') != '' ) {
        ...
        if ( is_single() && reisen_get_custom_option('show_reviews')=='yes' ) {
            wp_enqueue_script( 'reisen-core-reviews-script', trx_utils_get_file_url('js/core.reviews.js'), array('jquery'), null, true );
        }
    }
}
...
if (!function_exists('trx_utils_post_admin_scripts')) {
    function trx_utils_post_admin_scripts() {
        if (reisen_storage_get_array('post_override_options', 'page')=='post' && reisen_storage_isset('options', 'show_reviews'))
            wp_enqueue_script( 'reisen-core-reviews-script', trx_utils_get_file_url('js/core.reviews.js'), array('jquery'), null, true );
    }
}
    ...
    function trx_utils_get_privacy_text() {
        ...
        return apply_filters( 'trx_utils_filter_privacy_text', wp_kses_post(
                ...
                . ( '' != $page
                    ...
                    ? ' ' . sprintf(__('For f  urther details on handling user data, see our %s', 'trx_utils'),
...
if (!function_exists('reisen_footer_add_scroll_to_top')) {
    add_action('wp_footer', 'reisen_footer_add_scroll_to_top', 1);
    function reisen_footer_add_scroll_to_top() {
