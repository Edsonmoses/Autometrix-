...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('reisen_sc_slider_theme_setup')) {
	add_action( 'reisen_action_before_init_theme', 'reisen_sc_slider_theme_setup' );
	function reisen_sc_slider_theme_setup() {
		add_action('reisen_action_shortcodes_list', 		'reisen_sc_slider_reg_shortcodes');
		if (function_exists('reisen_exists_visual_composer') && reisen_exists_visual_composer())
			add_action('reisen_action_shortcodes_list_vc','reisen_sc_slider_reg_shortcodes_vc');
	}
}
...
if (!function_exists('reisen_sc_slider')) {	
	function reisen_sc_slider($atts, $content=null){	
		...
		extract(reisen_html_decode(shortcode_atts(array(
			...
			"slides_per_view" => 1,
			"titles" => "no",
			"descriptions" => reisen_get_custom_option('slider_info_descriptions'),
			"links" => "no",
			"align" => "",
			"interval" => "",
			"date_format" => "",
		...
		if (!empty($height) && reisen_param_is_on($autoheight)) $autoheight = "off";
		...
		$class2 = reisen_get_css_position_as_classes($top, $right, $bottom, $left);
		...
		$output = (in_array($pagination, array('full', 'over')) 
				...
				. ' class="sc_slider sc_slider_' . esc_attr($engine)
					...
					. (reisen_param_is_on($controls) ? ' sc_slider_controls' : ' sc_slider_nocontrols')
					. (reisen_param_is_on($pagination) ? ' sc_slider_pagination' : ' sc_slider_nopagination')
					. (reisen_storage_get_array('sc_slider_data', 'bg_image') ? ' sc_slider_bg' : ' sc_slider_images')
					. (!in_array($pagination, array('full', 'over')) 
							? ($class2 ? ' '.esc_attr($class2) : '') . ($align!='' && $align!='none' ? ' align'.esc_attr($align) : '')
							: '')
					. '"'
				. (!in_array($pagination, array('full', 'over')) && !reisen_param_is_off($animation) ? ' data-animation="'.esc_attr(reisen_get_animation_classes($animation)).'"' : '')
		...
		if ($engine=='revo') {
			...
			$output .= '<div class="slides'
				...
				.($engine=='swiper' && reisen_storage_get_array('sc_slider_data', 'bg_image') ? ' style="'.esc_attr($hs).'"' : '')
				.'>';
			...
			if (reisen_param_is_on($custom) && $content) {
				...
				$args = reisen_query_add_filters($args, 'thumbs');
				...
				while ( $query->have_posts() ) { 
					...
					if (reisen_param_is_on($crop)) {
						$post_attachment = reisen_storage_get_array('sc_slider_data', 'bg_image')
							? reisen_get_resized_image_url($post_attachment, !empty($width) && (float) $width.' ' == $width.' ' ? $width : null, !empty($height) && (float) $height.' ' == $height.' ' ? $height : null)
							: reisen_get_resized_image_tag($post_attachment, !empty($width) && (float) $width.' ' == $width.' ' ? $width : null, !empty($height) && (float) $height.' ' == $height.' ' ? $height : null);
					} else if (!reisen_storage_get_array('sc_slider_data', 'bg_image')) {
					...
					if (in_array($pagination, array('full', 'over'))) {
						$old_output = $output;
						$output = '';
						if (file_exists(reisen_get_file_dir('templates/_parts/widgets-posts.php'))) {
					...
					$output .= '<div' 
						...
						. ' style="'
							. (reisen_storage_get_array('sc_slider_data', 'bg_image') ? 'background-image:url(' . esc_url($post_attachment) . ');' : '') . ($ws) . ($hs)
							. '"'
						. '>' 
						. (reisen_param_is_on($links) ? '<a href="'.esc_url($post_link).'" title="'.esc_attr($post_title).'">' : '')
						. (!reisen_storage_get_array('sc_slider_data', 'bg_image') ? $post_attachment : '')
						;
					$caption = $engine=='swiper' ? '' : $caption;
					if (!reisen_param_is_off($titles)) {
						$post_hover_bg  = reisen_get_scheme_color('text_link');
						$post_bg = '';
						if ($post_hover_bg!='' && !reisen_is_inherit_option($post_hover_bg)) {
							$rgb = reisen_hex2rgb($post_hover_bg);
						...
						$post_descr = reisen_get_post_excerpt();
						if (reisen_get_custom_option("slider_info_category")=='yes') { // || empty($cat)) {
							// Get all post's categories
							$post_tax = reisen_get_taxonomy_categories_by_post_type($post_type);
							if (!empty($post_tax)) {
								$post_terms = reisen_get_terms_by_post_id(array('post_id'=>$post_id, 'taxonomy'=>$post_tax));
								if (!empty($post_terms[$post_tax])) {
									...
									if ($post_category!='') {
										$caption .= '<div class="sc_slider_category"'.(reisen_substr($post_accent_color, 0, 1)=='#' ? ' style="background-color: '.esc_attr($post_accent_color).'"' : '').'><a href="'.esc_url($post_category_link).'">'.($post_category).'</a></div>';
									}
								}
							}
						}
						$output_reviews = '';
						if (reisen_get_custom_option('show_reviews')=='yes' && reisen_get_custom_option('slider_info_reviews')=='yes') {
							$avg_author = reisen_reviews_marks_to_display(get_post_meta($post_id, reisen_storage_get('options_prefix').'_reviews_avg'.((reisen_get_theme_option('reviews_first')=='author' && $orderby != 'users_rating') || $orderby == 'author_rating' ? '' : '2'), true));
							if ($avg_author > 0) {
								$output_reviews .= '<div class="sc_slider_reviews post_rating reviews_summary blog_reviews' . (reisen_get_custom_option("slider_info_category")=='yes' ? ' after_category' : '') . '">'
									. '<div class="criteria_summary criteria_row">' . trim(reisen_reviews_get_summary_stars($avg_author, false, false, 5)) . '</div>'
									. '</div>';
							}
						}
						if (reisen_get_custom_option("slider_info_category")=='yes') $caption .= $output_reviews;
						$caption .= '<h3 class="sc_slider_subtitle"><a href="'.esc_url($post_link).'">'.($post_title).'</a></h3>';
						if (reisen_get_custom_option("slider_info_category")!='yes') $caption .= $output_reviews;
						if ($descriptions > 0) {
							$caption .= '<div class="sc_slider_descr">'.trim(reisen_strshort($post_descr, $descriptions)).'</div>';
						}
						$caption .= '</div>';
					}
					$output .= ($engine=='swiper' ? $caption : '') . (reisen_param_is_on($links) ? '</a>' : '' ) . '</div>';
				}
				wp_reset_postdata();
			}
		...
		return apply_filters('reisen_shortcode_output', $output, 'trx_slider', $atts, $content);
	}
	add_shortcode('trx_slider', 'reisen_sc_slider');
}


if (!function_exists('reisen_sc_slider_item')) {	
	function reisen_sc_slider_item($atts, $content=null) {
		if (reisen_in_shortcode_blogger()) return '';
		extract(reisen_html_decode(shortcode_atts( array(
			// Individual params
			"src" => "",
		...
		$engine = reisen_storage_get_array('sc_slider_data', 'engine');
		$crop_image = reisen_storage_get_array('sc_slider_data', 'crop_image');
		$bg_image = reisen_storage_get_array('sc_slider_data', 'bg_image');
		$links = reisen_storage_get_array('sc_slider_data', 'links');
		$width = reisen_storage_get_array('sc_slider_data', 'width');
		$height = reisen_storage_get_array('sc_slider_data', 'height');
	
		if ($src && reisen_param_is_on($crop_image)) {
			$src = reisen_storage_get_array('sc_slider_data', 'bg_image')
				? reisen_get_resized_image_url($src, !empty($width) && reisen_strpos($width, '%')===false ? $width : null, !empty($height) && reisen_strpos($height, '%')===false ? $height : null)
				: reisen_get_resized_image_tag($src, !empty($width) && reisen_strpos($width, '%')===false ? $width : null, !empty($height) && reisen_strpos($height, '%')===false ? $height : null);
		...
		$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '').' class="'.esc_attr($engine).'-slide' . (!empty($class) ? ' '.esc_attr($class) : '') . '"'
				. ($css ? ' style="'.esc_attr($css).'"' : '')
				.'>' 
				. ($src && reisen_param_is_on($links) ? '<a href="'.esc_url($src).'">' : '')
				. ($src && !$bg_image ? $src : $content)
				. ($src && reisen_param_is_on($links) ? '</a>' : '')
			. '</div>';
		return apply_filters('reisen_shortcode_output', $output, 'trx_slider_item', $atts, $content);
	}
	add_shortcode('trx_slider_item', 'reisen_sc_slider_item');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'reisen_sc_slider_reg_shortcodes' ) ) {
	//add_action('reisen_action_shortcodes_list', 'reisen_sc_slider_reg_shortcodes');
	function reisen_sc_slider_reg_shortcodes() {
	
		reisen_sc_map("trx_slider", array(
			...
			"params" => array_merge(array(
				"engine" => array(
					...
					"value" => "swiper",
					"type" => "checklist",
					"options" => reisen_get_sc_param('sliders')
				),
				"align" => array(
				...
				function_exists('reisen_exists_revslider') && reisen_exists_revslider() ? array(
				"alias" => array(
					...
					"value" => "",
					"type" => "select",
					"options" => reisen_get_sc_param('revo_sliders')
				)) : array(), array(
				"cat" => array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'reisen_sc_slider_reg_shortcodes_vc' ) ) {
	//add_action('reisen_action_shortcodes_list_vc', 'reisen_sc_slider_reg_shortcodes_vc');
	function reisen_sc_slider_reg_shortcodes_vc() {
		...
		vc_map( array(
			...
			"params" => array_merge(array(
				array(
					...
					"admin_label" => true,
					"class" => "",
					"value" => array_flip(reisen_get_sc_param('sliders')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "align",
				...
				array(
					...
					"value" => array_flip(reisen_array_merge(array('none' => esc_html__('- Select slider -', 'trx_utils')), reisen_get_sc_param('revo_sliders'))),
					"type" => "dropdown"
				)) : array(), array(
				array(
					"param_name" => "cat",
		...
		class WPBakeryShortCode_Trx_Slider extends Reisen_Vc_ShortCodeCollection {}
		class WPBakeryShortCode_Trx_Slider_Item extends Reisen_Vc_ShortCodeSingle {}
	}
}
?>