...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('reisen_sc_tabs_theme_setup')) {
	add_action( 'reisen_action_before_init_theme', 'reisen_sc_tabs_theme_setup' );
	function reisen_sc_tabs_theme_setup() {
		add_action('reisen_action_shortcodes_list', 		'reisen_sc_tabs_reg_shortcodes');
		if (function_exists('reisen_exists_visual_composer') && reisen_exists_visual_composer())
			add_action('reisen_action_shortcodes_list_vc','reisen_sc_tabs_reg_shortcodes_vc');
	}
}
...
if (!function_exists('reisen_sc_tabs')) {	
	function reisen_sc_tabs($atts, $content = null) {
		...
		reisen_storage_set('sc_tab_data', array(
			'counter'=> 0,
            'scroll' => $scroll,
		...
		$sc_tab_titles = reisen_storage_get_array('sc_tab_data', 'titles');
		...
		$tabs_output .= $titles_output
			...
			. ($content)
			.'</div>';
		return apply_filters('reisen_shortcode_output', $tabs_output, 'trx_tabs', $atts, $content);
	}
	add_shortcode("trx_tabs", "reisen_sc_tabs");
}


if (!function_exists('reisen_sc_tab')) {	
	function reisen_sc_tab($atts, $content = null) {
		if (reisen_in_shortcode_blogger()) return '';
		extract(reisen_html_decode(shortcode_atts(array(
			// Individual params
		...
		reisen_storage_inc_array('sc_tab_data', 'counter');
		$counter = reisen_storage_get_array('sc_tab_data', 'counter');
		if (empty($id))
			$id = !empty($tab_id) ? $tab_id : reisen_storage_get_array('sc_tab_data', 'id').'_'.intval($counter);
		$sc_tab_titles = reisen_storage_get_array('sc_tab_data', 'titles');
		...
		reisen_storage_set_array('sc_tab_data', 'titles', $sc_tab_titles);
		$output = '<div id="'.esc_attr($id).'"'
					.' class="sc_tabs_content' 
						...
						. ($css!='' ? ' style="'.esc_attr($css).'"' : '') 
						. '>' 
				. (reisen_param_is_on(reisen_storage_get_array('sc_tab_data', 'scroll')) 
					? '<div id="'.esc_attr($id).'_scroll" class="sc_scroll sc_scroll_vertical" style="height:'.(reisen_storage_get_array('sc_tab_data', 'height') != '' ? reisen_storage_get_array('sc_tab_data', 'height') : '200px').';"><div class="sc_scroll_wrapper swiper-wrapper"><div class="sc_scroll_slide swiper-slide">' 
					: '')
				...
				. (reisen_param_is_on(reisen_storage_get_array('sc_tab_data', 'scroll')) 
					...
					: '')
			. '</div>';
		return apply_filters('reisen_shortcode_output', $output, 'trx_tab', $atts, $content);
	}
	add_shortcode("trx_tab", "reisen_sc_tab");
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'reisen_sc_tabs_reg_shortcodes' ) ) {
	//add_action('reisen_action_shortcodes_list', 'reisen_sc_tabs_reg_shortcodes');
	function reisen_sc_tabs_reg_shortcodes() {
	
		reisen_sc_map("trx_tabs", array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'reisen_sc_tabs_reg_shortcodes_vc' ) ) {
	//add_action('reisen_action_shortcodes_list_vc', 'reisen_sc_tabs_reg_shortcodes_vc');
	function reisen_sc_tabs_reg_shortcodes_vc() {
		...
		class WPBakeryShortCode_Trx_Tabs extends Reisen_Vc_ShortCodeTabs {}
		class WPBakeryShortCode_Trx_Tab extends Reisen_Vc_ShortCodeTab {}
	}
}
?>