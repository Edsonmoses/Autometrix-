...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('reisen_sc_promo_theme_setup')) {
	add_action( 'reisen_action_before_init_theme', 'reisen_sc_promo_theme_setup' );
	function reisen_sc_promo_theme_setup() {
		add_action('reisen_action_shortcodes_list', 		'reisen_sc_promo_reg_shortcodes');
		if (function_exists('reisen_exists_visual_composer') && reisen_exists_visual_composer())
			add_action('reisen_action_shortcodes_list_vc','reisen_sc_promo_reg_shortcodes_vc');
	}
}
...
if (!function_exists('reisen_sc_promo')) {	
	function reisen_sc_promo($atts, $content=null){	
		...
		$text_width = reisen_strpos($image_width, '%')!==false
					...
					. '<div class="sc_promo_inner">'
						...
						. '<div class="sc_promo_block sc_align_'.esc_attr($text_align).'" style="'.esc_attr($css_text).'">'
							. '<div class="sc_promo_block_inner">'
									. (!empty($subtitle) ? '<h6 class="sc_promo_subtitle sc_item_subtitle">' . trim(reisen_strmacros($subtitle)) . '</h6>' : '')
									. (!empty($title) ? '<h2 class="sc_promo_title sc_item_title' . (empty($description) ? ' sc_item_title_without_descr' : ' sc_item_title_without_descr') . '">' . trim(reisen_strmacros($title)) . '</h2>' : '')
									. (!empty($description) ? '<div class="sc_promo_descr sc_item_descr">' . trim(reisen_strmacros($description)) . '</div>' : '')
									. (!empty($content) ? '<div class="sc_promo_content">'.do_shortcode($content).'</div>' : '')
									. (!empty($link) ? '<div class="sc_promo_button sc_item_button">'.reisen_do_shortcode('[trx_button link="'.esc_url($link).'" icon="icon-right"]'.esc_html($link_caption).'[/trx_button]').'</div>' : '')
							. '</div>'
						. '</div>'
					. '</div>'
				. '</div>';
		...
		return apply_filters('reisen_shortcode_output', $output, 'trx_promo', $atts, $content);
	}
	add_shortcode('trx_promo', 'reisen_sc_promo');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'reisen_sc_promo_reg_shortcodes' ) ) {
	//add_action('reisen_action_shortcodes_list', 'reisen_sc_promo_reg_shortcodes');
	function reisen_sc_promo_reg_shortcodes() {
	
		reisen_sc_map("trx_promo", array(
			...
			"params" => array(
				...
				"image_position" => array(
					...
					"type" => "checklist",
					"dir" => "horizontal",
					"options" => reisen_get_sc_param('hpos')
				),
				"image_width" => array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'reisen_sc_promo_reg_shortcodes_vc' ) ) {
	//add_action('reisen_action_shortcodes_list_vc', 'reisen_sc_promo_reg_shortcodes_vc');
	function reisen_sc_promo_reg_shortcodes_vc() {
		...
		vc_map( array(
			...
			"params" => array(
				...
				array(
					...
					"class" => "",
					"std" => 'left',
					"value" => array_flip(reisen_get_sc_param('hpos')),
					"type" => "dropdown"
				),
				array(
		...
		class WPBakeryShortCode_Trx_Promo extends Reisen_Vc_ShortCodeCollection {}
	}
}
?>