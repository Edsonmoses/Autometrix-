jQuery(document).ready(function () {
	...
	jQuery('#frontend_editor_icon_edit').on('click', function (e) {
		...
		jQuery('#frontend_editor_button_cancel').val(REISEN_STORAGE['strings']['editor_caption_cancel']);
	...
	jQuery('#frontend_editor_button_cancel').on('click', function (e) {
		"use strict";
		if (jQuery(this).val() == REISEN_STORAGE['strings']['editor_caption_close'])
	...
	jQuery('#frontend_editor_button_save').on('click', function (e) {
		...
		jQuery.post(REISEN_STORAGE['ajax_url'], data, function(response) {
			...
			if (rez.error == '') {
				reisen_message_success('', REISEN_STORAGE['strings']['editor_save_success']);
				jQuery('#frontend_editor_button_cancel').val(REISEN_STORAGE['strings']['editor_caption_close']);
			} else {
				reisen_message_warning(rez.error, REISEN_STORAGE['strings']['editor_save_error']);
			}
		});
		e.preventDefault();
		return false;
	...
	jQuery('#frontend_editor_icon_delete').on('click', function (e) {
		"use strict";
		reisen_message_confirm(REISEN_STORAGE['strings']['editor_delete_post'], REISEN_STORAGE['strings']['editor_delete_post_header'], function(btn) {
			"use strict";
			if (btn != 1) return;
			var data = {
			...
			jQuery.post(REISEN_STORAGE['ajax_url'], data, function(response) {
				...
				if (rez.error == '') {
					reisen_message_success('', REISEN_STORAGE['strings']['editor_delete_success']);
					setTimeout(function() { 
						window.location.href = REISEN_STORAGE['site_url'];
						}, 1000);
				} else {
					reisen_message_warning(rez.error, REISEN_STORAGE['strings']['editor_delete_error']);
				}
			});
