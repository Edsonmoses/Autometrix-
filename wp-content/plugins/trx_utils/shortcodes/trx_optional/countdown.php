...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('reisen_sc_countdown_theme_setup')) {
	add_action( 'reisen_action_before_init_theme', 'reisen_sc_countdown_theme_setup' );
	function reisen_sc_countdown_theme_setup() {
		add_action('reisen_action_shortcodes_list', 		'reisen_sc_countdown_reg_shortcodes');
		if (function_exists('reisen_exists_visual_composer') && reisen_exists_visual_composer())
			add_action('reisen_action_shortcodes_list_vc','reisen_sc_countdown_reg_shortcodes_vc');
	}
}
...
if (!function_exists('reisen_sc_countdown')) {	
	function reisen_sc_countdown($atts, $content = null) {
		...
		wp_enqueue_script( 'reisen-jquery-plugin-script', reisen_get_file_url('js/countdown/jquery.plugin.js'), array('jquery'), null, true );	
		wp_enqueue_script( 'reisen-countdown-script', reisen_get_file_url('js/countdown/jquery.countdown.js'), array('jquery'), null, true );	
		$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
			...
			. '>'
				...
				. ($align=='center' ? '</div>' : '')
			. '</div>';
		return apply_filters('reisen_shortcode_output', $output, 'trx_countdown', $atts, $content);
	}
	add_shortcode("trx_countdown", "reisen_sc_countdown");
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'reisen_sc_countdown_reg_shortcodes' ) ) {
	//add_action('reisen_action_shortcodes_list', 'reisen_sc_countdown_reg_shortcodes');
	function reisen_sc_countdown_reg_shortcodes() {
	
		reisen_sc_map("trx_countdown", array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'reisen_sc_countdown_reg_shortcodes_vc' ) ) {
	//add_action('reisen_action_shortcodes_list_vc', 'reisen_sc_countdown_reg_shortcodes_vc');
	function reisen_sc_countdown_reg_shortcodes_vc() {
		...
		class WPBakeryShortCode_Trx_Countdown extends Reisen_Vc_ShortCodeSingle {}
	}
}
?>