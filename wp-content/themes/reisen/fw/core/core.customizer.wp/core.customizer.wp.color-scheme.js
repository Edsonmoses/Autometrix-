/* global reisen_color_schemes, reisen_dependencies, Color */
...
( function( api ) {
	...
	if (!reisen_customizer_vars['need_refresh']) {
		for (var i in reisen_color_schemes) {
			cssTemplate[i] = wp.template( 'reisen-color-scheme-'+i );
		}
		cssTemplate['fonts'] = wp.template( 'reisen-fonts' );
	}
	...
	api.bind('ready', function() {
		...
		// Add 'Refresh' button
		if (reisen_customizer_vars['need_refresh']) {
			jQuery('#customize-header-actions .spinner').after('<input type="button" class="button customize-action-refresh" value="'+reisen_customizer_vars['msg_refresh']+'">');
		...
		jQuery('#customize-theme-controls .control-section').each(function () {
			reisen_customizer_wp_check_dependencies(jQuery(this));
		});

		// Update colors
		reisen_customizer_wp_change_color_scheme(api('color_scheme')());
	});
	
	// On change any control - check for dependencies
	api.bind('change', function(obj) {
		if (obj.id == 'scheme_storage') return;
		reisen_customizer_wp_check_dependencies(jQuery('#customize-theme-controls #customize-control-'+obj.id).parents('.control-section'));
		reisen_customizer_wp_refresh_preview(obj);
	});

	// Check for dependencies
	function reisen_customizer_wp_check_dependencies(cont) {
		"use strict";
		...
		cont.find('.customize-control').each(function() {
			...
			var depend = false;
			for (var fld in reisen_dependencies) {
				if (fld == id) {
					depend = reisen_dependencies[id];
					break;
				}
			}
			if (depend) {
				...
				if ((dep_cnt > 0 && dep_cmp == 'or') || (dep_cnt == dep_all && dep_cmp == 'and')) {
					jQuery(this).show().removeClass('reisen_options_no_use');
				} else {
					jQuery(this).hide().addClass('reisen_options_no_use');
				}
			}
		});
	}
	...
	function reisen_customizer_wp_refresh_preview(obj) {
		"use strict";
		...
		if (id == 'color_scheme') {
			reisen_customizer_wp_change_color_scheme(val);
		} else if (updateCSS) {
			var simple = api('color_settings')()=='simple';
			for (var opt in reisen_color_schemes['original']) {
				if (opt == id) {
					updateCSS = false;
					// Store new value in the color table
					reisen_customizer_wp_update_color_scheme(opt, val);
					...
					if (simple) {
						if (id == 'text_link') {
							...
							reisen_customizer_wp_update_color_scheme('alter_link', val);
						} else if (id == 'text_hover') {
							api('alter_hover').set( val );
							api.control( 'alter_hover' ).container.find( '.color-picker-hex' )
								.data( 'data-default-color', val )
								.wpColorPicker( 'defaultColor', val );
							reisen_customizer_wp_update_color_scheme('alter_hover', val);
						}
					}
					updateCSS = true;
					processed = true;
			...
			if (!processed) {
				for (var tag in reisen_fonts) {
					for (var prop in reisen_fonts[tag]) {
						if (prop=='title' || prop=='description') continue;
						if (tag+'-'+prop == id) {
							processed = true;
							// Store new value in the fonts table
							reisen_customizer_wp_update_fonts(tag, prop, val);
							break;
						}
					}
					if (processed) break;
			...
			// Refresh CSS
			if (processed) reisen_customizer_wp_update_css();
		}
	...
	// Store new value in the color table
	function reisen_customizer_wp_update_color_scheme(opt, value) {
		"use strict";
		reisen_color_schemes[api('color_scheme')()][opt] = value;
		api('scheme_storage').set(reisen_serialize(reisen_color_schemes))
	}
	

	// Change color scheme - update colors and generate css
	function reisen_customizer_wp_change_color_scheme(value) {
		"use strict";
		updateCSS = false;
		for (var opt in reisen_color_schemes[value]) {
			if (api(opt) == undefined) continue;
			api( opt ).set( reisen_color_schemes[value][opt] );
			api.control( opt ).container.find( '.color-picker-hex' )
				.data( 'data-default-color', reisen_color_schemes[value][opt] )
				.wpColorPicker( 'defaultColor', reisen_color_schemes[value][opt] );
		}
		updateCSS = true;
		reisen_customizer_wp_update_css();
	}

	// Store new value in the fonts table
	function reisen_customizer_wp_update_fonts(tag, prop, value) {
		"use strict";
		reisen_fonts[tag][prop] = value;
	}
	
	// Generate the CSS for the current Color Scheme and send it to the preview window
	function reisen_customizer_wp_update_css() {
		"use strict";

		if (!updateCSS || reisen_customizer_vars['need_refresh']) return;

		var css = '';
		
		// Add color styles
		for (var scheme in reisen_color_schemes) {
			
			var colors = [];
			
			// Copy all colors!
			for (var i in reisen_color_schemes[scheme]) {
				if (i=='title') continue;
				colors[i] = reisen_color_schemes[scheme][i];
			}
			
			// Make theme specific colors and tints
			colors = reisen_customizer_wp_add_theme_colors(colors);
		...
		for (var tag in reisen_fonts) {
			...
			fonts[tag+'_font-size'] = font['font-size'] && font['font-size']!='inherit'
												? 'font-size:' + reisen_prepare_css_value(font['font-size']) + ';'
												: '';
			fonts[tag+'_line-height'] = font['line-height'] && font['line-height']!='inherit'
												? 'line-height:' + reisen_prepare_css_value(font['line-height']) + ';'
												: '';
			fonts[tag+'_line-height_value'] = font['line-height']
												? reisen_prepare_css_value(font['line-height'])
												: 'inherit';
			...
			fonts[tag+'_margin-top'] = font['margin-top'] && font['margin-top']!='inherit'
												? "margin-top:" + reisen_prepare_css_value(font['margin-top']) + ";"
												: '';
			fonts[tag+'_margin-top_value'] = font['margin-top']
												? reisen_prepare_css_value(font['margin-top'])
												: 'inherit';
			fonts[tag+'_margin-bottom'] = font['margin-bottom'] && font['margin-bottom']!='inherit'
												? "margin-bottom:" + reisen_prepare_css_value(font['margin-bottom']) + ";"
												: '';
			fonts[tag+'_margin-bottom_value'] = font['margin-bottom']
												? reisen_prepare_css_value(font['margin-bottom'])
												: 'inherit';
		}
			
		// Make theme specific colors and tints
		if (window.reisen_customizer_wp_add_theme_fonts) fonts = reisen_customizer_wp_add_theme_fonts(fonts);
	...
	function reisen_customizer_wp_add_theme_colors(colors) {
	...
	// Attention! Don't forget setup custom colors also in the theme.styles.no_less.php
	function reisen_customizer_wp_add_theme_fonts(fonts) {
		fonts['logo_margin-top_0_75']	= reisen_multiply_css_value( fonts['logo_margin-top_value'], 0.75 );
		fonts['logo_margin-bottom_0_5']	= reisen_multiply_css_value( fonts['logo_margin-bottom_value'], 0.5 );
		fonts['menu_margin-top_1']		= fonts['menu_margin-top_value'];
		fonts['menu_margin-top_0_3']	= reisen_multiply_css_value( fonts['menu_margin-top_value'], 0.3 );
		fonts['menu_margin-top_0_6']	= reisen_multiply_css_value( fonts['menu_margin-top_value'], 0.6 );
		fonts['menu_margin-top_0_65']	= reisen_multiply_css_value( fonts['menu_margin-top_value'], 0.65 );
		fonts['menu_margin-top_0_7']	= reisen_multiply_css_value( fonts['menu_margin-top_value'], 0.7 );
		fonts['menu_margin-top_0_8']	= reisen_multiply_css_value( fonts['menu_margin-top_value'], 0.8 );
		fonts['menu_margin-bottom_1']	= fonts['menu_margin-bottom_value'];
		fonts['menu_margin-bottom_0_5']	= reisen_multiply_css_value( fonts['menu_margin-bottom_value'], 0.5 );
		fonts['menu_margin-bottom_0_6']	= reisen_multiply_css_value( fonts['menu_margin-bottom_value'], 0.6 );
		fonts['menu_height_1']			= reisen_summ_css_value( reisen_summ_css_value( fonts['menu_margin-top_value'],  fonts['menu_margin-bottom_value'] ), fonts['menu_line-height_value'] );
