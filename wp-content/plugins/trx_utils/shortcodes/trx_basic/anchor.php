...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('reisen_sc_anchor_theme_setup')) {
	add_action( 'reisen_action_before_init_theme', 'reisen_sc_anchor_theme_setup' );
	function reisen_sc_anchor_theme_setup() {
		add_action('reisen_action_shortcodes_list', 		'reisen_sc_anchor_reg_shortcodes');
		if (function_exists('reisen_exists_visual_composer') && reisen_exists_visual_composer())
			add_action('reisen_action_shortcodes_list_vc','reisen_sc_anchor_reg_shortcodes_vc');
	}
}
...
if (!function_exists('reisen_sc_anchor')) {	
	function reisen_sc_anchor($atts, $content = null) {
		...
		$output = $id 
			? '<a id="'.esc_attr($id).'"'
				...
				. ' data-description="' . ($description ? esc_attr(reisen_strmacros($description)) : ''). '"'
				. ' data-icon="' . ($icon ? $icon : '') . '"' 
				. ' data-url="' . ($url ? esc_attr($url) : '') . '"' 
				. ' data-separator="' . (reisen_param_is_on($separator) ? 'yes' : 'no') . '"'
				. '></a>'
			: '';
		return apply_filters('reisen_shortcode_output', $output, 'trx_anchor', $atts, $content);
	}
	add_shortcode("trx_anchor", "reisen_sc_anchor");
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'reisen_sc_anchor_reg_shortcodes' ) ) {
	//add_action('reisen_action_shortcodes_list', 'reisen_sc_anchor_reg_shortcodes');
	function reisen_sc_anchor_reg_shortcodes() {
	
		reisen_sc_map("trx_anchor", array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'reisen_sc_anchor_reg_shortcodes_vc' ) ) {
	//add_action('reisen_action_shortcodes_list_vc', 'reisen_sc_anchor_reg_shortcodes_vc');
	function reisen_sc_anchor_reg_shortcodes_vc() {
		...
		class WPBakeryShortCode_Trx_Anchor extends Reisen_Vc_ShortCodeSingle {}
	}
}
?>