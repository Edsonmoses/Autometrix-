...
if ( !function_exists( 'reisen_template_testimonials_1_theme_setup' ) ) {
	add_action( 'reisen_action_before_init_theme', 'reisen_template_testimonials_1_theme_setup', 1 );
	function reisen_template_testimonials_1_theme_setup() {
		reisen_add_template(array(
			'layout' => 'testimonials-1',
			'template' => 'testimonials-1',
			'mode'   => 'testimonials',
			'title'  => esc_html__('Testimonials /Style 1/', 'reisen'),
			'thumb_title'  => esc_html__('Testimonials image', 'reisen'),
			'w'		 => 370,
			'h'		 => 240
		));
	}
...
// Template output
if ( !function_exists( 'reisen_template_testimonials_1_output' ) ) {
	function reisen_template_testimonials_1_output($post_options, $post_data) {
		$show_title = true;
		...
		?>
			<div<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?> class="sc_testimonial_item<?php echo (!empty($post_options['tag_class']) ? ' '.esc_attr($post_options['tag_class']) : ''); ?>"<?php echo !empty($post_options['tag_css']) ? ' style="'.esc_attr($post_options['tag_css']).'"' : '';?>>
                ...
                <?php if ($post_options['photo']) { ?>
                    <div class="sc_testimonial_image"><?php reisen_show_layout($post_options['photo']); ?></div>
                <?php } ?>

                <div class="sc_testimonial_content"><?php reisen_show_layout(wp_kses_data($post_data['post_content'])); ?>
