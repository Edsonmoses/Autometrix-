...
if ( !function_exists( 'reisen_template_single_standard_theme_setup' ) ) {
	add_action( 'reisen_action_before_init_theme', 'reisen_template_single_standard_theme_setup', 1 );
	function reisen_template_single_standard_theme_setup() {
		reisen_add_template(array(
			'layout' => 'single-standard',
			'mode'   => 'single',
			'need_content' => true,
			'need_terms' => true,
			'title'  => esc_html__('Single standard', 'reisen'),
			'thumb_title'  => esc_html__('Fullwidth image (crop)', 'reisen'),
			'w'		 => 1170,
			'h'		 => 659
		));
	}
...
// Template output
if ( !function_exists( 'reisen_template_single_standard_output' ) ) {
	function reisen_template_single_standard_output($post_options, $post_data) {
		...
		if ($show_title && $post_options['location'] == 'center' && reisen_get_custom_option('show_page_title')=='no') {
			?>
			<<?php echo esc_attr($title_tag); ?> itemprop="<?php echo (float) $avg_author > 0 || (float) $avg_users > 0 ? 'itemReviewed' : 'headline'; ?>" class="post_title entry-title"><span class="post_icon <?php echo esc_attr($post_data['post_icon']); ?>"></span><?php reisen_show_layout($post_data['post_title']); ?></<?php echo esc_attr($title_tag); ?>>
		<?php 
		}
		...
		if ($show_title && $post_options['location'] != 'center' && reisen_get_custom_option('show_page_title')=='no') {
			?>
			<<?php echo esc_attr($title_tag); ?> itemprop="<?php echo (float) $avg_author > 0 || (float) $avg_users > 0 ? 'itemReviewed' : 'headline'; ?>" class="post_title entry-title"><span class="post_icon <?php echo esc_attr($post_data['post_icon']); ?>"></span><?php reisen_show_layout(wp_kses_data($post_data['post_title'])); ?></<?php echo esc_attr($title_tag); ?>>
			<?php 
		}
		...
		if ($post_data['post_protected']) { 
			...
			if (!reisen_storage_empty('reviews_markup') && function_exists('reisen_sc_reviews') && reisen_strpos($post_data['post_content'], reisen_get_reviews_placeholder())===false && function_exists('reisen_sc_reviews'))
            ...
            if(function_exists('reisen_sc_reviews')){
                ...
                reisen_show_layout(reisen_gap_wrapper($post_data['post_content']));
            }
			wp_link_pages( array( 
			...
			if ( reisen_get_custom_option('show_post_tags') == 'yes' && !empty($post_data['post_terms'][$post_data['post_taxonomy_tags']]->terms_links)) {
				...
				<div class="post_info post_info_bottom">
					<span class="post_info_item post_info_tags"><span class="btag"><?php esc_html_e('Tags:', 'reisen'); ?></span> <?php echo join(', ', $post_data['post_terms'][$post_data['post_taxonomy_tags']]->terms_links); ?></span>
				</div>
				<?php 
			}
		} 
		...
		$sidebar_present = !reisen_param_is_off(reisen_get_custom_option('show_sidebar_main'));
		if (!$sidebar_present) reisen_close_wrapper();
