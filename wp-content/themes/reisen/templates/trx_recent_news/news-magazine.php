...
if ( !function_exists( 'reisen_template_news_magazine_theme_setup' ) ) {
	add_action( 'reisen_action_before_init_theme', 'reisen_template_news_magazine_theme_setup', 1 );
	function reisen_template_news_magazine_theme_setup() {
		reisen_add_template(array(
			'layout' => 'news-magazine',
			'template' => 'news-magazine',
			'mode'   => 'news',
			'title'  => esc_html__('Recent News /Style Magazine/', 'reisen'),
...
// Template output
if ( !function_exists( 'reisen_template_news_magazine_output' ) ) {
	function reisen_template_news_magazine_output($post_options, $post_data) {
				...
				<div class="post_header entry-header">
					...
					if ($show_title) {
						if (!isset($post_options['links']) || $post_options['links']) {
							?>
							<<?php echo esc_attr($title_tag); ?> class="post_title entry-title"><a href="<?php echo esc_url($post_data['post_link']); ?>"><?php reisen_show_layout($post_data['post_title']); ?></a></<?php echo esc_attr($title_tag); ?>>
							<?php
						} else {
							?>
							<<?php echo esc_attr($title_tag); ?> class="post_title entry-title"><?php reisen_show_layout(wp_kses_data($post_data['post_title'])); ?></<?php echo esc_attr($title_tag); ?>>
							<?php
						}
					}
					
					if ( in_array( $post_data['post_type'], array( 'post', 'attachment' ) ) ) {
						?><div class="post_meta"><span class="post_meta_author"><?php reisen_show_layout(wp_kses_data($post_data['post_author_link'])); ?></span><?php
					...
					if ($post_data['post_protected']) {
						...
						if ($post_data['post_excerpt']) {
							echo in_array($post_data['post_format'], array('quote', 'link', 'chat', 'aside', 'status')) 
									...
									: wpautop(reisen_strshort($post_data['post_excerpt'], isset($post_options['descr']) 
																								? $post_options['descr'] 
