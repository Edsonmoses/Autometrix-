...
if ( !function_exists( 'reisen_template_list_theme_setup' ) ) {
	add_action( 'reisen_action_before_init_theme', 'reisen_template_list_theme_setup', 1 );
	function reisen_template_list_theme_setup() {
		reisen_add_template(array(
			'layout' => 'list',
			'mode'   => 'blogger',
			'need_columns' => true,
			'title'  => esc_html__('Blogger layout: List', 'reisen')
			));
	}
}

// Template output
if ( !function_exists( 'reisen_template_list_output' ) ) {
	function reisen_template_list_output($post_options, $post_data) {
