...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('reisen_sc_audio_theme_setup')) {
	add_action( 'reisen_action_before_init_theme', 'reisen_sc_audio_theme_setup' );
	function reisen_sc_audio_theme_setup() {
		add_action('reisen_action_shortcodes_list', 		'reisen_sc_audio_reg_shortcodes');
		if (function_exists('reisen_exists_visual_composer') && reisen_exists_visual_composer())
			add_action('reisen_action_shortcodes_list_vc','reisen_sc_audio_reg_shortcodes_vc');
	}
}
...
if (!function_exists('reisen_sc_audio')) {
	function reisen_sc_audio($atts, $content = null) {
		...
		$audio = '<audio'
			...
			. ' src="'.esc_url($src).'"'
			. (reisen_param_is_on($controls) ? ' controls="controls"' : '')
			. (reisen_param_is_on($autoplay) && is_single() ? ' autoplay="autoplay"' : '')
			. ($css!='' ? ' style="'.esc_attr($css).'"' : '')
			. ($data)
			. '></audio>';
		if ( reisen_get_custom_option('substitute_audio')=='no') {
			if (reisen_param_is_on($frame)) {
				$audio = reisen_get_audio_frame($audio, $image, $s);
			}
		} else {
			if ((isset($_GET['vc_editable']) && $_GET['vc_editable']=='true') && (isset($_POST['action']) && $_POST['action']=='vc_load_shortcode')) {
				$audio = reisen_substitute_audio($audio, false);
			}
		}
		if (reisen_get_theme_option('use_mediaelement')=='yes')
			wp_enqueue_script('wp-mediaelement');
		return apply_filters('reisen_shortcode_output', $audio, 'trx_audio', $atts, $content);
	}
	add_shortcode("trx_audio", "reisen_sc_audio");
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'reisen_sc_audio_reg_shortcodes' ) ) {
	//add_action('reisen_action_shortcodes_list', 'reisen_sc_audio_reg_shortcodes');
	function reisen_sc_audio_reg_shortcodes() {
	
		reisen_sc_map("trx_audio", array(
			...
			"params" => array(
				...
				"controls" => array(
					...
					"value" => "show",
					"type" => "switch",
					"options" => reisen_get_sc_param('show_hide')
				),
				"autoplay" => array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'reisen_sc_audio_reg_shortcodes_vc' ) ) {
	//add_action('reisen_action_shortcodes_list_vc', 'reisen_sc_audio_reg_shortcodes_vc');
	function reisen_sc_audio_reg_shortcodes_vc() {
		...
		class WPBakeryShortCode_Trx_Audio extends Reisen_Vc_ShortCodeSingle {}
	}
}
?>