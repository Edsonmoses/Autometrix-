...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('reisen_sc_columns_theme_setup')) {
	add_action( 'reisen_action_before_init_theme', 'reisen_sc_columns_theme_setup' );
	function reisen_sc_columns_theme_setup() {
		add_action('reisen_action_shortcodes_list', 		'reisen_sc_columns_reg_shortcodes');
		if (function_exists('reisen_exists_visual_composer') && reisen_exists_visual_composer())
			add_action('reisen_action_shortcodes_list_vc','reisen_sc_columns_reg_shortcodes_vc');
	}
}
...
if (!function_exists('reisen_sc_columns')) {	
	function reisen_sc_columns($atts, $content=null){	
		...
		reisen_storage_set('sc_columns_data', array(
			'counter' => 1,
			'equal_selector' => '',
            'after_span2' => false,
            'after_span3' => false,
		...
		$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') 
				. ' class="columns_wrap sc_columns'
					. ' columns_' . (reisen_param_is_on($fluid) ? 'fluid' : 'nofluid') 
					. (!empty($margins) && reisen_param_is_off($margins) ? ' no_margins' : '') 
					. ' sc_columns_count_' . esc_attr($count)
					. (!empty($class) ? ' '.esc_attr($class) : '') 
				. '"'
				. ($css!='' ? ' style="'.esc_attr($css).'"' : '')
				. (!reisen_param_is_off($equalheight) ? ' data-equal-height="'.esc_attr(reisen_storage_get_array('sc_columns_data', 'equal_selector')).'"' : '')
				. (!reisen_param_is_off($animation) ? ' data-animation="'.esc_attr(reisen_get_animation_classes($animation)).'"' : '')
				. '>'
					. trim($content)
				. '</div>';
		return apply_filters('reisen_shortcode_output', $output, 'trx_columns', $atts, $content);
	}
	add_shortcode('trx_columns', 'reisen_sc_columns');
}


if (!function_exists('reisen_sc_column_item')) {	
	function reisen_sc_column_item($atts, $content=null) {
		if (reisen_in_shortcode_blogger()) return '';
		extract(reisen_html_decode(shortcode_atts( array(
			// Individual params
			"span" => "1",
		...
		$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') . ' class="column-'.($span > 1 ? esc_attr($span) : 1).'_'.esc_attr(reisen_storage_get_array('sc_columns_data', 'count')).' sc_column_item sc_column_item_'.esc_attr(reisen_storage_get_array('sc_columns_data', 'counter')) 
					. (!empty($class) ? ' '.esc_attr($class) : '')
					. (reisen_storage_get_array('sc_columns_data', 'counter') % 2 == 1 ? ' odd' : ' even') 
					. (reisen_storage_get_array('sc_columns_data', 'counter') == 1 ? ' first' : '') 
					. ($span > 1 ? ' span_'.esc_attr($span) : '') 
					. (reisen_storage_get_array('sc_columns_data', 'after_span2') ? ' after_span_2' : '') 
					. (reisen_storage_get_array('sc_columns_data', 'after_span3') ? ' after_span_3' : '') 
					. (reisen_storage_get_array('sc_columns_data', 'after_span4') ? ' after_span_4' : '') 
					. '"'
					. ($css!='' ? ' style="'.esc_attr($css).'"' : '')
					. (!reisen_param_is_off($animation) ? ' data-animation="'.esc_attr(reisen_get_animation_classes($animation)).'"' : '')
					. '>'
					...
					. '</div>';
		reisen_storage_inc_array('sc_columns_data', 'counter', $span);
		reisen_storage_set_array('sc_columns_data', 'after_span2', $span==2);
		reisen_storage_set_array('sc_columns_data', 'after_span3', $span==3);
		reisen_storage_set_array('sc_columns_data', 'after_span4', $span==4);
		reisen_storage_set_array('sc_columns_data', 'equal_selector', $bg_color!=='' || $bg_image !== '' ? '.sc_column_item_inner' : '.sc_column_item');
		return apply_filters('reisen_shortcode_output', $output, 'trx_column_item', $atts, $content);
	}
	add_shortcode('trx_column_item', 'reisen_sc_column_item');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'reisen_sc_columns_reg_shortcodes' ) ) {
	//add_action('reisen_action_shortcodes_list', 'reisen_sc_columns_reg_shortcodes');
	function reisen_sc_columns_reg_shortcodes() {
	
		reisen_sc_map("trx_columns", array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'reisen_sc_columns_reg_shortcodes_vc' ) ) {
	//add_action('reisen_action_shortcodes_list_vc', 'reisen_sc_columns_reg_shortcodes_vc');
	function reisen_sc_columns_reg_shortcodes_vc() {
		...
		class WPBakeryShortCode_Trx_Columns extends Reisen_Vc_ShortCodeColumns {}
		class WPBakeryShortCode_Trx_Column_Item extends Reisen_Vc_ShortCodeCollection {}
	}
}
?>