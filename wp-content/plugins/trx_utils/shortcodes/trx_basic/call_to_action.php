...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('reisen_sc_call_to_action_theme_setup')) {
	add_action( 'reisen_action_before_init_theme', 'reisen_sc_call_to_action_theme_setup' );
	function reisen_sc_call_to_action_theme_setup() {
		add_action('reisen_action_shortcodes_list', 		'reisen_sc_call_to_action_reg_shortcodes');
		if (function_exists('reisen_exists_visual_composer') && reisen_exists_visual_composer())
			add_action('reisen_action_shortcodes_list_vc','reisen_sc_call_to_action_reg_shortcodes_vc');
	}
}
...
if (!function_exists('reisen_sc_call_to_action')) {	
	function reisen_sc_call_to_action($atts, $content=null){	
		...
		if (!empty($image)) {
			$thumb_sizes = reisen_get_thumb_sizes(array('layout' => 'excerpt'));
			$image = !empty($video) 
				? reisen_get_resized_image_url($image, $thumb_sizes['w'], $thumb_sizes['h']) 
				: reisen_get_resized_image_tag($image, $thumb_sizes['w'], $thumb_sizes['h']);
		}
	
		if (!empty($video)) {
			$video = '<video' . ($id ? ' id="' . esc_attr($id.'_video') . '"' : '') 
				. ' class="sc_video"'
				. ' src="' . esc_url(reisen_get_video_player_url($video)) . '"'
				...
				. '>'
				. '</video>';
			if (reisen_get_custom_option('substitute_video')=='no') {
				$video = reisen_get_video_frame($video, $image, '', '');
			} else {
				if ((isset($_GET['vc_editable']) && $_GET['vc_editable']=='true') && (isset($_POST['action']) && $_POST['action']=='vc_load_shortcode')) {
					$video = reisen_substitute_video($video, $width, $height, false);
				}
			}
		...
		$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') 
				. ' class="sc_call_to_action'
					. (reisen_param_is_on($accent) ? ' sc_call_to_action_accented' : '')
			...
			. '>'
				...
				. '<div class="sc_call_to_action_info'.($need_columns ? ' column-'.esc_attr($need_columns-1).'_'.esc_attr($need_columns) : '').'">'
					. (!empty($subtitle) ? '<h6 class="sc_call_to_action_subtitle sc_item_subtitle">' . trim(reisen_strmacros($subtitle)) . '</h6>' : '')
					. (!empty($title) ? '<h2 class="sc_call_to_action_title sc_item_title' . (empty($description) ? ' sc_item_title_without_descr' : ' sc_item_title_without_descr') . '">' . trim(reisen_strmacros($title)) . '</h2>' : '')
					. (!empty($description) ? '<div class="sc_call_to_action_descr sc_item_descr">' . trim(reisen_strmacros($description)) . '</div>' : '')
					. ($style==1 ? $buttons : '')
				. '</div>'
				. ($style==2 && $align!='right' ? $buttons : '')
				. ($align=='right' ? $featured : '')
		...
		return apply_filters('reisen_shortcode_output', $output, 'trx_call_to_action', $atts, $content);
	}
	add_shortcode('trx_call_to_action', 'reisen_sc_call_to_action');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'reisen_sc_call_to_action_reg_shortcodes' ) ) {
	//add_action('reisen_action_shortcodes_list', 'reisen_sc_call_to_action_reg_shortcodes');
	function reisen_sc_call_to_action_reg_shortcodes() {
	
		reisen_sc_map("trx_call_to_action", array(
			...
			"params" => array(
				...
				"style" => array(
					...
					"value" => "1",
					"type" => "checklist",
					"options" => reisen_get_list_styles(1, 2)
				),
				"align" => array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'reisen_sc_call_to_action_reg_shortcodes_vc' ) ) {
	//add_action('reisen_action_shortcodes_list_vc', 'reisen_sc_call_to_action_reg_shortcodes_vc');
	function reisen_sc_call_to_action_reg_shortcodes_vc() {
		...
		vc_map( array(
			...
			"params" => array(
				array(
					...
					"class" => "",
					"admin_label" => true,
					"value" => array_flip(reisen_get_list_styles(1, 2)),
					"type" => "dropdown"
				),
				array(
					"param_name" => "align",
		...
		class WPBakeryShortCode_Trx_Call_To_Action extends Reisen_Vc_ShortCodeCollection {}
	}
}
?>