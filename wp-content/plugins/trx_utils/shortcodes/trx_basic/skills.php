...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('reisen_sc_skills_theme_setup')) {
	add_action( 'reisen_action_before_init_theme', 'reisen_sc_skills_theme_setup' );
	function reisen_sc_skills_theme_setup() {
		add_action('reisen_action_shortcodes_list', 		'reisen_sc_skills_reg_shortcodes');
		if (function_exists('reisen_exists_visual_composer') && reisen_exists_visual_composer())
			add_action('reisen_action_shortcodes_list_vc','reisen_sc_skills_reg_shortcodes_vc');
	}
}
...
if (!function_exists('reisen_sc_skills')) {	
	function reisen_sc_skills($atts, $content=null){	
		...
		reisen_storage_set('sc_skills_data', array(
			'counter' => 0,
            'columns' => 0,
            'height'  => 0,
            'type'    => $type,
            'pie_compact' => reisen_param_is_on($pie_compact) ? 'on' : 'off',
		...
		reisen_enqueue_diagram($type);
		if ($type!='arc') {
			if ($layout=='' || ($layout=='columns' && $columns<1)) $layout = 'rows';
			if ($layout=='columns') reisen_storage_set_array('sc_skills_data', 'columns', $columns);
			if ($type=='bar') {
		...
		if ($style) {
			...
			reisen_storage_set_array('sc_skills_data', 'style', $style);
		}
		reisen_storage_set_array('sc_skills_data', 'max', $max_value);
		reisen_storage_set_array('sc_skills_data', 'dir', $dir);
		reisen_storage_set_array('sc_skills_data', 'height', reisen_prepare_css_value($height));
		$class .= ($class ? ' ' : '') . reisen_get_css_position_as_classes($top, $right, $bottom, $left);
		$css .= reisen_get_css_dimensions_from_values($width);
		if (!reisen_storage_empty('sc_skills_data', 'height') && (reisen_storage_get_array('sc_skills_data', 'type') == 'arc' || (reisen_storage_get_array('sc_skills_data', 'type') == 'pie' && reisen_param_is_on(reisen_storage_get_array('sc_skills_data', 'pie_compact')))))
			$css .= 'height: '.reisen_storage_get_array('sc_skills_data', 'height');
		$content = do_shortcode($content);
		$output = '<div id="'.esc_attr($id).'"' 
					. ' class="sc_skills sc_skills_' . esc_attr($type) 
						. ($type=='bar' ? ' sc_skills_'.esc_attr($dir) : '') 
						. ($type=='pie' ? ' sc_skills_compact_'.esc_attr(reisen_storage_get_array('sc_skills_data', 'pie_compact')) : '') 
				...
				. '>'
					...
					. ($type=='arc' 
						? ('<div class="sc_skills_legend">'.(reisen_storage_get_array('sc_skills_data', 'legend')).'</div>'
							. '<div id="'.esc_attr($id).'_diagram" class="sc_skills_arc_canvas"></div>'
							. '<div class="sc_skills_data" style="display:none;">' . (reisen_storage_get_array('sc_skills_data', 'data')) . '</div>'
						  )
						: '')
					. ($type=='pie' && reisen_param_is_on(reisen_storage_get_array('sc_skills_data', 'pie_compact'))
						? ('<div class="sc_skills_legend">'.(reisen_storage_get_array('sc_skills_data', 'legend')).'</div>'
				...
				. '</div>';
		return apply_filters('reisen_shortcode_output', $output, 'trx_skills', $atts, $content);
	}
	add_shortcode('trx_skills', 'reisen_sc_skills');
}


if (!function_exists('reisen_sc_skills_item')) {	
	function reisen_sc_skills_item($atts, $content=null) {
		if (reisen_in_shortcode_blogger()) return '';
		extract(reisen_html_decode(shortcode_atts( array(
			// Individual params
			"title" => "",
		...
		reisen_storage_inc_array('sc_skills_data', 'counter');
		$ed = reisen_substr($value, -1)=='%' ? '%' : '';
		$value = str_replace('%', '', $value);
		if (reisen_storage_get_array('sc_skills_data', 'max') < $value) reisen_storage_set_array('sc_skills_data', 'max', $value);
		$percent = round($value / reisen_storage_get_array('sc_skills_data', 'max') * 100);
		$start = 0;
		$stop = $value;
		$steps = 100;
		$step = max(1, round(reisen_storage_get_array('sc_skills_data', 'max')/$steps));
		$speed = mt_rand(10,40);
		$animation = round(($stop - $start) / $step * $speed);
		$title_block = '<div class="sc_skills_info"><div class="sc_skills_label">' . ($title) . '</div></div>';
		$old_color = $color;
		if (empty($color)) $color = reisen_storage_get_array('sc_skills_data', 'color');
		if (empty($color)) $color = reisen_get_scheme_color('text_link', $color);
		if (empty($bg_color)) $bg_color = reisen_storage_get_array('sc_skills_data', 'bg_color');
		if (empty($bg_color)) $bg_color = reisen_get_scheme_color('bg_color', $bg_color);
		if (empty($border_color)) $border_color = reisen_storage_get_array('sc_skills_data', 'border_color');
		if (empty($border_color)) $border_color = reisen_get_scheme_color('bd_color', $border_color);;
		if (empty($style)) $style = reisen_storage_get_array('sc_skills_data', 'style');
		$style = max(1, min(4, $style));
		$output = '';
		if (reisen_storage_get_array('sc_skills_data', 'type') == 'arc' || (reisen_storage_get_array('sc_skills_data', 'type') == 'pie' && reisen_param_is_on(reisen_storage_get_array('sc_skills_data', 'pie_compact')))) {
			if (reisen_storage_get_array('sc_skills_data', 'type') == 'arc' && empty($old_color)) {
				$rgb = reisen_hex2rgb($color);
				$color = 'rgba('.(int)$rgb['r'].','.(int)$rgb['g'].','.(int)$rgb['b'].','.(1 - 0.1*(reisen_storage_get_array('sc_skills_data', 'counter')-1)).')';
			}
			reisen_storage_concat_array('sc_skills_data', 'legend', 
			...
			reisen_storage_concat_array('sc_skills_data', 'data', 
				'<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
					. ' class="'.esc_attr(reisen_storage_get_array('sc_skills_data', 'type')).'"'
					. (reisen_storage_get_array('sc_skills_data', 'type')=='pie'
						? ( ' data-start="'.esc_attr($start).'"'
							. ' data-stop="'.esc_attr($stop).'"'
							. ' data-step="'.esc_attr($step).'"'
							. ' data-steps="'.esc_attr($steps).'"'
							. ' data-max="'.esc_attr(reisen_storage_get_array('sc_skills_data', 'max')).'"'
							...
							. ' data-cutout="'.esc_attr(reisen_storage_get_array('sc_skills_data', 'pie_cutout')).'"'
			...
			$output .= (reisen_storage_get_array('sc_skills_data', 'columns') > 0 
							? '<div class="sc_skills_column column-1_'.esc_attr(reisen_storage_get_array('sc_skills_data', 'columns')).'">' 
							: '')
					. (reisen_storage_get_array('sc_skills_data', 'type')=='bar' && reisen_storage_get_array('sc_skills_data', 'dir')=='horizontal' ? $title_block : '')
					. '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') 
						. ' class="sc_skills_item' . ($style ? ' sc_skills_style_'.esc_attr($style) : '') 
							. (!empty($class) ? ' '.esc_attr($class) : '')
							. (reisen_storage_get_array('sc_skills_data', 'counter') % 2 == 1 ? ' odd' : ' even') 
							. (reisen_storage_get_array('sc_skills_data', 'counter') == 1 ? ' first' : '') 
							. '"'
						. (reisen_storage_get_array('sc_skills_data', 'height') !='' || $css 
							? ' style="' 
								. (reisen_storage_get_array('sc_skills_data', 'height') !='' 
										? 'height: '.esc_attr(reisen_storage_get_array('sc_skills_data', 'height')).';' 
										: '') 
								. ($css) 
								. '"' 
							: '')
			...
			if (in_array(reisen_storage_get_array('sc_skills_data', 'type'), array('bar', 'counter'))) {
				$output .= '<div class="sc_skills_count"' . (reisen_storage_get_array('sc_skills_data', 'type')=='bar' && $color ? ' style="background-color:' . esc_attr($color) . '; border-color:' . esc_attr($color) . '"' : '') . '>'
							...
							.'</div>'
						. '</div>';
			} else if (reisen_storage_get_array('sc_skills_data', 'type')=='pie') {
			...
			$output .= 
					  (reisen_storage_get_array('sc_skills_data', 'type')=='counter' ? $title_block : '')
					. '</div>'
					. (reisen_storage_get_array('sc_skills_data', 'type')=='bar' && reisen_storage_get_array('sc_skills_data', 'dir')=='vertical' || reisen_storage_get_array('sc_skills_data', 'type') == 'pie' ? $title_block : '')
					. (reisen_storage_get_array('sc_skills_data', 'columns') > 0 ? '</div>' : '');
		}
		return apply_filters('reisen_shortcode_output', $output, 'trx_skills_item', $atts, $content);
	}
	add_shortcode('trx_skills_item', 'reisen_sc_skills_item');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'reisen_sc_skills_reg_shortcodes' ) ) {
	//add_action('reisen_action_shortcodes_list', 'reisen_sc_skills_reg_shortcodes');
	function reisen_sc_skills_reg_shortcodes() {
	
		reisen_sc_map("trx_skills", array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'reisen_sc_skills_reg_shortcodes_vc' ) ) {
	//add_action('reisen_action_shortcodes_list_vc', 'reisen_sc_skills_reg_shortcodes_vc');
	function reisen_sc_skills_reg_shortcodes_vc() {
		...
		class WPBakeryShortCode_Trx_Skills extends Reisen_Vc_ShortCodeCollection {}
		class WPBakeryShortCode_Trx_Skills_Item extends Reisen_Vc_ShortCodeSingle {}
	}
}
?>