...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('reisen_sc_highlight_theme_setup')) {
	add_action( 'reisen_action_before_init_theme', 'reisen_sc_highlight_theme_setup' );
	function reisen_sc_highlight_theme_setup() {
		add_action('reisen_action_shortcodes_list', 		'reisen_sc_highlight_reg_shortcodes');
		if (function_exists('reisen_exists_visual_composer') && reisen_exists_visual_composer())
			add_action('reisen_action_shortcodes_list_vc','reisen_sc_highlight_reg_shortcodes_vc');
	}
}
...
if (!function_exists('reisen_sc_highlight')) {	
	function reisen_sc_highlight($atts, $content=null){	
		...
		$css .= ($color != '' ? 'color:' . esc_attr($color) . ';' : '')
			...
			.($font_size != '' ? 'font-size:' . esc_attr(reisen_prepare_css_value($font_size)) . '; line-height: 1em;' : '');
		$output = '<span' . ($id ? ' id="'.esc_attr($id).'"' : '') 
				...
				. '</span>';
		return apply_filters('reisen_shortcode_output', $output, 'trx_highlight', $atts, $content);
	}
	add_shortcode('trx_highlight', 'reisen_sc_highlight');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'reisen_sc_highlight_reg_shortcodes' ) ) {
	//add_action('reisen_action_shortcodes_list', 'reisen_sc_highlight_reg_shortcodes');
	function reisen_sc_highlight_reg_shortcodes() {
	
		reisen_sc_map("trx_highlight", array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'reisen_sc_highlight_reg_shortcodes_vc' ) ) {
	//add_action('reisen_action_shortcodes_list_vc', 'reisen_sc_highlight_reg_shortcodes_vc');
	function reisen_sc_highlight_reg_shortcodes_vc() {
		...
		class WPBakeryShortCode_Trx_Highlight extends Reisen_Vc_ShortCodeSingle {}
	}
}
?>